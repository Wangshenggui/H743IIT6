...
INIT_BSP_LATE(bsp_mem_init);

//内存池(64字节对齐)
__align(64) uint8_t mem1base[MEM1_MAX_SIZE];												//内部SRAM内存池
__align(64) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((at(0XC0000000)));				//外部SDRAM内存池,前面2M给LTDC用了(1280*800*2)

//内存管理表
uint32_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];												//内部SRAM内存池MAP
uint32_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((at(0XC0000000 + MEM2_MAX_SIZE)));	//外部SDRAM内存池MAP
//内存管理参数
